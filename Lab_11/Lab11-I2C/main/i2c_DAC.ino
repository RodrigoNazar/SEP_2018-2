...
void send_data(uint8_t msb_data, uint8_t lsb_data)
{
	...
	i2cSendByte(MCP4725_WRITE); 			// send WRITE address of MCP4725
	i2cWaitForComplete();
	i2cSendByte(msb_data); 					// Send first data byte
	i2cWaitForComplete();
	i2cSendByte(lsb_data); 					// Send second data byte
	i2cWaitForComplete();
...
// Fórmula de la salida del DAC está en la página 19
// Vout = (Vdd*N)/4096
