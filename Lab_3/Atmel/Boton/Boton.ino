...
int main(){
 ...
 // PB7 y PD1 son imputs, al activarle las resistencias de pull up
  ...
  while(1){
    ...
    if ((!(PINB & (1<<boton))) | (!(PIND & (1<<boton2)))) //Queremos que el boton esté low (!) PINB es el registro de lectura, 
    { //lo comparamos con el estado en que estamos interesados, (1<<PINB7)
      _delay_ms (10);
      if ((!(PINB & (1<<boton))) | (!(PIND & (1<<boton2))))
    {
      _delay_ms (10);
      if ((!(PINB & (1<<boton))) | (!(PIND & (1<<boton2))))
    {
      ...
      while ((!(PINB & (1<<boton))) | (!(PIND & (1<<boton2)))){  // No cambia de estado si se sigue presionando el boton
      }
